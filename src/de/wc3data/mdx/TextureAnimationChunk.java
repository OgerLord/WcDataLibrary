...
public class TextureAnimationChunk {
	...
	public class TextureAnimation {
		...
		public void load(BlizzardDataInputStream in) throws IOException {
			...
			for (int i = 0; i < 3; i++) {
				if (StreamUtils.checkOptionalId(in, textureTranslation.key)) {
					textureTranslation = new TextureTranslation();
					textureTranslation.load(in);
				} else if (StreamUtils.checkOptionalId(in, textureRotation.key)) {
					textureRotation = new TextureRotation();
					textureRotation.load(in);
				} else if (StreamUtils.checkOptionalId(in, textureScaling.key)) {
