...
public class Node {
	...
	public void load(BlizzardDataInputStream in) throws IOException {
		...
		flags = in.readInt();
		for (int i = 0; i < 3; i++) {
			if (StreamUtils.checkOptionalId(in, geosetTranslation.key)) {
				geosetTranslation = new GeosetTranslation();
				geosetTranslation.load(in);
			} else if (StreamUtils.checkOptionalId(in, geosetRotation.key)) {
				geosetRotation = new GeosetRotation();
				geosetRotation.load(in);
			} else if (StreamUtils.checkOptionalId(in, geosetScaling.key)) {
