...
public class GeosetRotation {
	...
	public class RotationTrack {
		...
		public void load(BlizzardDataInputStream in) throws IOException {
			time = in.readInt();
			rotation = StreamUtils.loadFloatArray(in, 4);
			if (interpolationType > 1) {
				inTan = StreamUtils.loadFloatArray(in, 4);
				outTan = StreamUtils.loadFloatArray(in, 4);
			}
		}
		...
		public void save(BlizzardDataOutputStream out) throws IOException {
			...
			StreamUtils.saveFloatArray(out, rotation);
			if (interpolationType > 1) {
				if (inTan.length % 4 != 0) {
					throw new IllegalArgumentException(
